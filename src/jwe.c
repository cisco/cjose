...
static bool _cjose_jwe_aes_kw_wrap(const uint8_t *kek,
                                   size_t kek_len,
                                   const uint8_t *plain,
                                   size_t plain_len,
                                   uint8_t *wrapped,
...
static bool _cjose_jwe_aes_kw_unwrap(const uint8_t *kek,
                                     size_t kek_len,
...
static bool _cjose_jwe_decrypt_ek_aes_kw(_jwe_int_recipient_t *recipient, cjose_jwe_t *jwe, const cjose_jwk_t *jwk, cjose_err *err)
{
    ...
    if (!_cjose_jwe_aes_kw_unwrap(jwk->keydata, jwk->keysize / 8, recipient->enc_key.raw, recipient->enc_key.raw_len,
                                   jwe->cek, &jwe->cek_len))
    {
        CJOSE_ERROR(err, CJOSE_ERR_CRYPTO);
        return false;
    ...
    _jwe_int_recipient_t *recipient, cjose_jwe_t *jwe, const cjose_jwk_t *jwk, int padding, const EVP_MD *oaep_md, cjose_err *err)
{
    ...
    if (ctx == NULL || EVP_PKEY_encrypt_init(ctx) != 1 || EVP_PKEY_CTX_set_rsa_padding(ctx, padding) != 1
        || (oaep_md != NULL && (EVP_PKEY_CTX_set_rsa_oaep_md(ctx, oaep_md) != 1
    ...
    _jwe_int_recipient_t *recipient, cjose_jwe_t *jwe, const cjose_jwk_t *jwk, int padding, const EVP_MD *oaep_md, cjose_err *err)
{
    ...
    EVP_PKEY *key = _cjose_jwk_rsa_key(jwk);
    if (!_cjose_jwk_rsa_has_private(key))
    {
        CJOSE_ERROR(err, CJOSE_ERR_INVALID_ARG);
        return false;
    }
    ...
    _jwe_int_recipient_t *recipient, cjose_jwe_t *jwe, const cjose_jwk_t *jwk, const char *alg, size_t kek_keysize, cjose_err *err)
{
    ...
    if (!_cjose_jwe_aes_kw_wrap(kek, kek_keysize, jwe->cek, jwe->cek_len, recipient->enc_key.raw,
                                 &recipient->enc_key.raw_len))
    {
        CJOSE_ERROR(err, CJOSE_ERR_CRYPTO);
        goto cjose_encrypt_ek_ecdh_es_kw_finish;
    ...
    _jwe_int_recipient_t *recipient, cjose_jwe_t *jwe, const cjose_jwk_t *jwk, const char *alg, size_t kek_keysize, cjose_err *err)
{
    ...
    if (!_cjose_jwe_aes_kw_unwrap(kek, kek_keysize, recipient->enc_key.raw, recipient->enc_key.raw_len, jwe->cek,
                                   &jwe->cek_len))
    {
        CJOSE_ERROR(err, CJOSE_ERR_CRYPTO);
        goto cjose_decrypt_ek_ecdh_es_kw_finish;
